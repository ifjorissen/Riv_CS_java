...
public class Cube
{
    ...
    public Cubelet [] [] [] getCubeArray()
    {
        return cube;
    }

    public Cubelet getCubie (int x, int y, int z)
    {
        return cube[z][y][x];   
    }
    ...
    private void setLocationsColors()
    {
        for (int i = 0; i < cube.length; i++) //iterate through outermost aspect of array
        {
            for (int j = 0; j < cube[0].length; j++)
            {
                ...
                {
                    cube[i][j][k].setLocation(k*2.1, j*2.1, i*2.1); //i,j,k  is z, y, x not x,y,z
    ...
    {  
        ...
        for (int j = 0; j<cube[0].length; j++)
        {
            for (int k = 0; k<cube[0][0].length; k++)
            {
                if (k == 0)
                {
                    double x = cube[0][j][k].getLocation()[0];
                    double y = cube[0][j][k].getLocation()[1];
